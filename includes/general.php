...
function tk_general_init(): void {
    ...
    add_action('admin_post_tk_general_sitemap_submit', 'tk_general_sitemap_submit');
    ...
    if ((int) tk_get_option('duplicate_content_enabled', 0) === 1) {
        add_filter('post_row_actions', 'tk_general_duplicate_row_action', 10, 2);
        add_filter('page_row_actions', 'tk_general_duplicate_row_action', 10, 2);
        add_action('admin_post_tk_duplicate_content', 'tk_general_duplicate_content_handler');
    }
...
function tk_general_duplicate_row_action(array $actions, $post): array {
    if (
        !is_object($post)
    ...
    $url = wp_nonce_url(
        admin_url('admin-post.php?action=tk_duplicate_content&post=' . (int) $post->ID),
        'tk_duplicate_content_' . (int) $post->ID
    );
    $actions['tk_duplicate'] = '<a href="' . esc_url($url) . '">' . esc_html__('Duplicate', 'tool-kits') . '</a>';
    return $actions;
}

function tk_general_duplicate_post(int $post_id) {
    $source = get_post($post_id);
    if (!$source || !in_array($source->post_type, array('post', 'page'), true)) {
        return new WP_Error('tk_duplicate_invalid_post', __('The selected content cannot be duplicated.', 'tool-kits'));
    }
    ...
    $duplicate_id = wp_insert_post(
        array(
            ...
            'post_title' => sprintf(__('Copy of %s', 'tool-kits'), $source->post_title),
            'post_status' => 'draft',
    ...
    $polylang_active = function_exists('pll_get_post_language') && function_exists('pll_set_post_language');
    foreach (get_object_taxonomies($source->post_type) as $taxonomy) {
        if ($polylang_active && in_array($taxonomy, array('language', 'post_translations'), true)) {
            continue;
        }
    ...
    $excluded_meta = array('_edit_lock', '_edit_last', '_wp_old_slug', '_icl_lang_duplicate_of');
    ...
    tk_general_duplicate_language($post_id, (int) $duplicate_id, $source->post_type);

    return (int) $duplicate_id;
}

function tk_general_duplicate_language(int $source_id, int $duplicate_id, string $post_type): void {
    if (function_exists('pll_get_post_language') && function_exists('pll_set_post_language')) {
        $language = (string) pll_get_post_language($source_id, 'slug');
        if ($language !== '') {
            pll_set_post_language($duplicate_id, $language);
        }
        return;
    }
    ...
    $details = apply_filters('wpml_post_language_details', null, $source_id);
    $language_code = is_array($details) && isset($details['language_code']) ? (string) $details['language_code'] : '';
    if ($language_code === '') {
        return;
    }
...
function tk_general_duplicate_content_handler(): void {
    $post_id = isset($_GET['post']) ? absint($_GET['post']) : 0;
    check_admin_referer('tk_duplicate_content_' . $post_id);

    if ($post_id <= 0 || !current_user_can('edit_post', $post_id)) {
        wp_die(__('You are not allowed to duplicate this content.', 'tool-kits'));
    }

    $duplicate_id = tk_general_duplicate_post($post_id);
    if (is_wp_error($duplicate_id)) {
        wp_die(esc_html($duplicate_id->get_error_message()));
    }

    wp_safe_redirect(get_edit_post_link($duplicate_id, 'raw'));
    exit;
}
...
function tk_general_sitemap_url(): string {
    $path = trim((string) tk_get_option('seo_sitemap_path', 'sitemap.xml'));
    return home_url('/' . ltrim($path !== '' ? $path : 'sitemap.xml', '/'));
}

function tk_general_sitemap_submit_google(string $sitemap_url) {
    $token = trim((string) tk_get_option('sitemap_google_access_token', ''));
    $property = trim((string) tk_get_option('sitemap_google_property', home_url('/')));
    if ($token === '' || $property === '') {
        return new WP_Error('tk_sitemap_google_credentials', __('Google Search Console property and access token are required.', 'tool-kits'));
    }
    $endpoint = 'https://www.googleapis.com/webmasters/v3/sites/' . rawurlencode($property) . '/sitemaps/' . rawurlencode($sitemap_url);
...
function tk_general_sitemap_submit_bing(string $sitemap_url) {
    $token = trim((string) tk_get_option('sitemap_bing_access_token', ''));
    if ($token === '') {
        return new WP_Error('tk_sitemap_bing_credentials', __('Bing Webmaster OAuth access token is required.', 'tool-kits'));
    }
    return wp_remote_post('https://www.bing.com/webmaster/api.svc/json/SubmitFeed', array(
        'timeout' => 20,
        'headers' => array(
...
function tk_general_sitemap_submission_result(string $provider, $response): array {
    $result = array(
        'provider' => $provider,
        'submitted_at' => time(),
        'ok' => false,
    ...
    $result['message'] = $result['ok']
        ? __('Sitemap submitted successfully.', 'tool-kits')
        : sprintf(__('Submission failed with HTTP status %d.', 'tool-kits'), $status);
    return $result;
}

function tk_general_sitemap_submit(): void {
    tk_require_admin_post('tk_general_sitemap_submit');
    $provider = sanitize_key((string) tk_post('provider', ''));
    $sitemap_url = tk_general_sitemap_url();
    if ($provider === 'google') {
        $response = tk_general_sitemap_submit_google($sitemap_url);
    } elseif ($provider === 'bing') {
        $response = tk_general_sitemap_submit_bing($sitemap_url);
    } else {
        wp_die(esc_html__('Unknown sitemap provider.', 'tool-kits'));
    }
    $result = tk_general_sitemap_submission_result($provider, $response);
    tk_update_option('sitemap_submission_last_result', $result);
    wp_safe_redirect(add_query_arg(array(
        'page' => 'tool-kits-general',
        'tk_sitemap_submit' => $result['ok'] ? 'ok' : 'fail',
        'tk_sitemap_provider' => $provider,
...
function tk_render_general_page(): void {
    ...
    $duplicate_content_enabled = (int) tk_get_option('duplicate_content_enabled', 0);
    ...
    $sitemap_google_property = (string) tk_get_option('sitemap_google_property', '');
    if ($sitemap_google_property === '') {
        $sitemap_google_property = home_url('/');
    }
    $sitemap_google_connected = trim((string) tk_get_option('sitemap_google_access_token', '')) !== '';
    $sitemap_bing_connected = trim((string) tk_get_option('sitemap_bing_access_token', '')) !== '';
    $sitemap_submission = tk_get_option('sitemap_submission_last_result', array());
    $sitemap_submission = is_array($sitemap_submission) ? $sitemap_submission : array();
    $sitemap_submit_status = isset($_GET['tk_sitemap_submit']) ? sanitize_key((string) $_GET['tk_sitemap_submit']) : '';
    $sitemap_submit_provider = isset($_GET['tk_sitemap_provider']) ? sanitize_key((string) $_GET['tk_sitemap_provider']) : '';
        ...
        <div class="tk-tabs tk-general-tabs">
            ...
            <div class="tk-tabs-content">
                <div class="tk-card tk-tab-panel is-active" data-panel-id="editing">
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        ...
                        <div style="display:flex; flex-direction:column; gap:20px;">
                            ...
                            tk_render_switch('duplicate_content_enabled', 'Enable Duplicate Page/Post', 'Adds a Duplicate action to Posts and Pages. Copies are always created as drafts.', $duplicate_content_enabled);
                            ?>
                        </div>
                ...
                <div class="tk-card tk-tab-panel" data-panel-id="xml-sitemap">
                    ...
                    <?php endif; ?>
                    <?php if ($sitemap_submit_status === 'ok') : ?>
                        <?php tk_notice(ucfirst($sitemap_submit_provider) . ' accepted the sitemap submission.', 'success'); ?>
                    <?php elseif ($sitemap_submit_status === 'fail') : ?>
                        <?php tk_notice(ucfirst($sitemap_submit_provider) . ' sitemap submission failed. Review the result below and refresh the OAuth token if needed.', 'error'); ?>
                    <?php endif; ?>
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        <?php tk_nonce_field('tk_general_save'); ?>
                        <input type="hidden" name="action" value="tk_general_save">
                        ...
                        <div style="display:flex; flex-direction:column; gap:20px;">
                            ...
                            <div class="tk-sitemap-integrations">
                                <h3>Sitemap Submission</h3>
                                <p class="description">Connect verified webmaster properties. Access tokens are stored but never printed back into this page.</p>
                                <div class="tk-grid tk-grid-2">
                                    <div class="tk-sitemap-provider">
                                        <h4>Google Search Console</h4>
                                        <label>Property URL</label>
                                        <input type="text" name="sitemap_google_property" value="<?php echo esc_attr($sitemap_google_property); ?>" placeholder="https://example.com/ or sc-domain:example.com">
                                        <label>OAuth Access Token</label>
                                        <input type="password" name="sitemap_google_access_token" value="" autocomplete="new-password" placeholder="<?php echo $sitemap_google_connected ? 'Connected - enter a token to replace it' : 'Paste access token'; ?>">
                                        <?php if ($sitemap_google_connected) : ?>
                                            <label class="tk-checkbox-line"><input type="checkbox" name="sitemap_google_token_clear" value="1"> Remove saved Google token</label>
                                        <?php endif; ?>
                                    </div>
                                    <div class="tk-sitemap-provider">
                                        <h4>Bing Webmaster Tools</h4>
                                        <label>OAuth Access Token</label>
                                        <input type="password" name="sitemap_bing_access_token" value="" autocomplete="new-password" placeholder="<?php echo $sitemap_bing_connected ? 'Connected - enter a token to replace it' : 'Paste access token'; ?>">
                                        <?php if ($sitemap_bing_connected) : ?>
                                            <label class="tk-checkbox-line"><input type="checkbox" name="sitemap_bing_token_clear" value="1"> Remove saved Bing token</label>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="tk-sitemap-submit-panel">
                        <h4>Submit Generated Sitemap</h4>
                        <p class="description"><code><?php echo esc_html($sitemap_url); ?></code></p>
                        <div class="tk-inline">
                            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                                <?php tk_nonce_field('tk_general_sitemap_submit'); ?>
                                <input type="hidden" name="action" value="tk_general_sitemap_submit">
                                <input type="hidden" name="provider" value="google">
                                <button class="button button-secondary" <?php disabled(!$sitemap_google_connected); ?>>Submit to Google</button>
                            </form>
                            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                                <?php tk_nonce_field('tk_general_sitemap_submit'); ?>
                                <input type="hidden" name="action" value="tk_general_sitemap_submit">
                                <input type="hidden" name="provider" value="bing">
                                <button class="button button-secondary" <?php disabled(!$sitemap_bing_connected); ?>>Submit to Bing</button>
                            </form>
                        </div>
                        <?php if (!empty($sitemap_submission)) : ?>
                            <p class="description">
                                Last result:
                                <strong><?php echo esc_html(ucfirst((string) ($sitemap_submission['provider'] ?? ''))); ?></strong>,
                                <?php echo !empty($sitemap_submission['ok']) ? 'success' : 'failed'; ?>,
                                <?php echo !empty($sitemap_submission['submitted_at']) ? esc_html(wp_date('Y-m-d H:i:s', (int) $sitemap_submission['submitted_at'])) : '-'; ?>.
                                <?php echo esc_html((string) ($sitemap_submission['message'] ?? '')); ?>
                            </p>
                        <?php endif; ?>
                    </div>
                </div>
...
function tk_general_save(): void {
    ...
    if ($section === 'editing') {
        ...
        tk_update_option('duplicate_content_enabled', !empty($_POST['duplicate_content_enabled']) ? 1 : 0);
        ...
        tk_update_option('sitemap_google_property', sanitize_text_field((string) tk_post('sitemap_google_property', home_url('/'))));
        if (!empty($_POST['sitemap_google_token_clear'])) {
            tk_update_option('sitemap_google_access_token', '');
        } elseif (trim((string) tk_post('sitemap_google_access_token', '')) !== '') {
            tk_update_option('sitemap_google_access_token', sanitize_text_field((string) tk_post('sitemap_google_access_token', '')));
        }
        if (!empty($_POST['sitemap_bing_token_clear'])) {
            tk_update_option('sitemap_bing_access_token', '');
        } elseif (trim((string) tk_post('sitemap_bing_access_token', '')) !== '') {
            tk_update_option('sitemap_bing_access_token', sanitize_text_field((string) tk_post('sitemap_bing_access_token', '')));
        }
