...
function tk_db_export_with_pairs(array $pairs): array {
    ...
    $sql_path = $dir . 'toolkits-db-preload-' . date('dmY-His') . '.sql';
    $gz_path = $sql_path . '.gz';
    ...
    if ($gz === false) {
        @unlink($sql_path);
        return ['ok' => false, 'message' => 'Failed gz encoding export.'];
    }
...
function tk_render_db_migrate_page() {
    ...
    $allowed_tabs = array('export-db','preload-export','change-prefix');
    ...
    (function(){
        ...
        function attachPairs(wrapper) {
            var body = wrapper.querySelector('tbody');
            var addBtn = wrapper.querySelector('.tk-add-row');
            var findName = wrapper.getAttribute('data-find-name') || 'pairs_find';
            var replaceName = wrapper.getAttribute('data-replace-name') || 'pairs_replace';
            if (!body) { return; }
        ...
        function initTabs() {
            var buttons = document.querySelectorAll('.tk-tabs-nav-button');
            buttons.forEach(function(button){
                button.addEventListener('click', function(){
                    activateTab(button.getAttribute('data-panel'));
                });
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', function(){
            document.querySelectorAll('.tk-find-replace-wrapper').forEach(function(wrapper){
                attachPairs(wrapper);
            });
            initTabs();
...
function tk_db_export_handler() {
    ...
    $filename = 'wp-db-export-' . date('Y-m-d-His') . '.sql';
...
function tk_db_run_find_replace_handler() {
    ...
    if (!$result['ok']) {
        wp_redirect(add_query_arg(array('page'=>'tool-kits-db-migrate','tk_err'=>'export_failed','tk_err_msg'=>$result['message'],'tk_tab'=>'preload-export'), admin_url('admin.php')));
        exit;
    }

    wp_redirect(add_query_arg(array(
        'page' => 'tool-kits-db-migrate',
...
function tk_db_change_prefix_handler() {
    ...
    if ($new === '') {
        wp_redirect(add_query_arg(array('page'=>'tool-kits-db-migrate','tk_err'=>'prefix_empty','tk_tab'=>'change-prefix'), admin_url('admin.php')));
        exit;
    }

    $old = $wpdb->prefix;
    if ($new === $old) {
        wp_redirect(add_query_arg(array('page'=>'tool-kits-db-migrate','tk_err'=>'prefix_same','tk_tab'=>'change-prefix'), admin_url('admin.php')));
        exit;
    }
    ...
    if (empty($tables)) {
        wp_redirect(add_query_arg(array('page'=>'tool-kits-db-migrate','tk_err'=>'no_tables','tk_tab'=>'change-prefix'), admin_url('admin.php')));
        exit;
    }
    ...
    if (!empty($errors)) {
        $wpdb->query('ROLLBACK');
        $msg = implode('; ', $errors);
        wp_redirect(add_query_arg(array('page'=>'tool-kits-db-migrate','tk_err'=>'rename_failed','tk_err_msg'=>$msg,'tk_tab'=>'change-prefix'), admin_url('admin.php')));
        exit;
    }
    ...
    $wpdb->query('COMMIT');
        ...
        'page'=>'tool-kits-db-migrate',
        'tk_ok' => 1,
        'tk_msg' => 'Prefix renamed; update wp-config.php accordingly.',
