...
add_filter('auto_update_plugin', 'tk_github_allow_auto_update', 9999, 2);
add_action('load-plugins.php', 'tk_github_load_plugins_page');

$GLOBALS['tk_github_updating_this_plugin'] = false;

function tk_github_updater_enabled(): bool {
    return !defined('TK_GITHUB_UPDATER_ENABLED') || TK_GITHUB_UPDATER_ENABLED === true;
}

function tk_github_can_check_remote(): bool {
    return tk_github_updater_enabled() && current_user_can('update_plugins')
        && !wp_doing_cron() && !wp_doing_ajax()
        && (!empty($GLOBALS['tk_github_manual_check']) || (is_admin() && ($GLOBALS['pagenow'] ?? '') === 'plugins.php'));
}

function tk_github_load_plugins_page(): void {
    if (!tk_github_can_check_remote()) { return; }
    $state = get_site_transient('update_plugins');
    if (is_object($state)) {
        set_site_transient('update_plugins', tk_github_plugin_update_check($state));
    }
}

function tk_github_allow_auto_update($update, $item) {
    if (!tk_github_updater_enabled() && is_object($item)
        && ($item->plugin ?? '') === plugin_basename(TK_PATH . 'tool-kits.php')) {
        return false;
    }
    return $update;
}
...
function tk_github_fetch_latest_release() {
    if (!tk_github_updater_enabled()) {
        return new WP_Error('tk_github_updater_disabled', 'Tool Kits GitHub updater is temporarily disabled. Install updates manually while diagnosing server errors.');
    }
    ...
    if (!tk_github_can_check_remote()) {
        return new WP_Error('tk_github_check_deferred', 'GitHub checks run only on the Plugins page.');
    }
    if (get_transient('tk_github_check_cooldown')) {
        return new WP_Error('tk_github_check_cooldown', 'GitHub check recently attempted. Retry after five minutes.');
    }
    ...
    // Back off after failures even when no release could be cached.
    set_transient('tk_github_check_cooldown', 1, 5 * MINUTE_IN_SECONDS);
...
function tk_github_plugin_action_links(array $links): array {
    if (!tk_github_updater_enabled()) {
        $links[] = '<span>GitHub Updater paused</span>';
        return $links;
    }
    $url = wp_nonce_url(
...
function tk_github_check_now_handler(): void {
    ...
    if (!tk_github_updater_enabled() || get_transient('tk_github_check_cooldown')) {
        wp_die('Updater unavailable or recently checked. Retry after five minutes.');
    }
    $GLOBALS['tk_github_manual_check'] = true;
