...
function tk_captcha_render_markup(): string {
    ...
    $letterSpacing = mt_rand(3, 7);
    $fontWeight = mt_rand(700, 900);
    ...
    $html  = '<div class="tk-captcha-field" style="margin:12px 0;display:flex;flex-direction:column;gap:8px;">';
    $html .= '<label style="display:block;font-weight:600;margin-bottom:4px;color:#f8fafc;">Enter the code below</label>';
    $html .= '<div style="display:flex;flex-wrap:wrap;gap:12px;align-items:flex-start;background:#020812;border-radius:14px;padding:14px;border:1px solid rgba(255,255,255,0.18);box-shadow:0 12px 20px rgba(2,6,23,.75);">';
    $html .= '<div style="min-width:160px;display:flex;flex-wrap:wrap;gap:4px;align-items:center;background:' . esc_attr($gradient) . ', ' . esc_attr($noiseSvg) . ';padding:10px 14px;border-radius:8px 14px;border:1px solid rgba(255,255,255,0.5);box-shadow:inset 0 -2px 6px rgba(0,0,0,.3);text-transform:none;">';
    $html .= $charSpans;
    $html .= '</div>';
    $html .= '<input type="text" name="' . esc_attr($names['answer']) . '" autocomplete="off" placeholder="Type the captcha" aria-label="Captcha code" style="flex:1;min-width:210px;padding:12px 16px;border-radius:10px;border:1px solid rgba(148,163,184,0.4);background:#030712;background-image:' . esc_attr($bgNoise) . ';color:#f8fafc;font-weight:600;">';
    $html .= '<button type="button" class="button tk-captcha-refresh" style="margin-left:8px;margin-top:4px;padding:8px 14px;border-radius:8px;background:#0f172a;color:#f8fafc;border:1px solid rgba(248,250,252,0.4);">Refresh</button>';
    $html .= '</div>';
    $html .= '<div style="display:flex;flex-direction:column;gap:3px;font-size:12px;color:#94a3b8;margin-top:12px;">';
    $html .= '<span>Typing the code exactly, including case, is required.</span>';
    $html .= '<span style="font-size:11px;color:#94a3b8;">Codes expire in 5 minutes to keep brute-force attackers at bay.</span>';
    $html .= '</div>';
...
function tk_captcha_refresh_script() {
    ...
    (function(){
        ...
        document.addEventListener('click', function(e){
            if (!e.target.classList.contains('tk-captcha-refresh')) {
                return;
            }
            e.preventDefault();
            var btn = e.target;
...
function tk_captcha_save() {
    ...
    wp_redirect(add_query_arg(array('page'=>'tool-kits-security-captcha','tk_saved'=>1), admin_url('admin.php')));
    exit;
}
